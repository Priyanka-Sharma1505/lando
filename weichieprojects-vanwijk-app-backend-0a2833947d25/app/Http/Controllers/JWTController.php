...
class JWTController extends Controller
{
    ...
    protected function responseWithTokenAndUser($token, $user){
        return response()->json([
            'access_token' => $token,
            'expires_in' => auth('api')->factory()->getTTL() * 60,
            'user' => $user->only('id', 'odoo_user_id', 'odoo', 'email', 'first_name', 'last_name', 'all_permissions', 'is_account', 'is_admin', 'is_super_admin', 'is_client', 'is_portal', 'locations', 'addressRequests', 'addressRequests.request'),
